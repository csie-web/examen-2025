...
class TaskStore {
  ...
  async getAll (state, projectId, pageNumber = '', pageSize = '', filterField = '', filterValue = '', sortField = '', sortOrder = '') {
    try {
      const response = await fetch(`${SERVER}/api/users/${state.user.data.id}/projects/${projectId}/tasks?pageSize=${pageSize || ''}&pageNumber=${pageNumber === '' ? 0 : pageNumber}&filterField=${filterField || ''}&filterValue=${filterValue || ''}&sortField=${sortField || ''}&sortOrder=${sortOrder || ''}`, {
        headers: {
